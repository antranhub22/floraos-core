...
| 02 | `02-function-catalog.md` | Từ vựng quyền — 113 năng lực |
...
**Một sự thật, một chỗ.** Bảng 76 mã chỉ có ở tài liệu 02. Lược đồ chỉ có ở 07 và `prisma/schema.prisma`. Lộ trình chỉ có ở kiến trúc V2 mục 15. Các tệp khác dẫn chiếu, không chép lại.

**Con số sinh từ mã nguồn.** Bảng năng lực ở tài liệu 02 sinh từ `maChucNang.ts`. Hình dạng hợp đồng Vision đọc từ `Schema.json`. Danh sách cột `assets` đọc từ cơ sở dữ liệu thật của SocialFlow. Con số nào không trích được từ đâu thì không nằm trong bộ tài liệu này.
