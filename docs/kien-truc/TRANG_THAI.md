...
**0. Chạy `npm run test:tenant` trên Terminal Mac thật — cổng DUY NHẤT chưa xác minh.**
Phiên soát 09/10 chạy được bốn cổng còn lại (`tsc`/`eslint`/`npm test` 141/141/`pytest` 43/43 và
`npm run build`), nhưng không chạy được cổng này: VM `device_bash` không có Docker, và khi dựng
Postgres chạy tay được thì `prisma db push` chặn ở `binaries.prisma.sh` (403 policy denial, không
lấy được engine `linux-arm64`). Đợt sửa 09/10 thêm 10 ca mới nên số dự kiến là **79** ca, không
còn là 69.
...
9. **P8 — chạy lượt nạp AVI GIFT thật trên Postgres:**
   ...
   npx tsx scripts/nap-credit.ts --org-id=<uuid script vừa in> --amount=<số credit>
   ```
