...
app.floraos.vn/marketing/*       → proxy sang SocialFlow (M04b)
app.floraos.vn/social/*          → proxy sang SocialFlow (M07)
...
| B3 | `SocialFlow`: middleware FastAPI xác thực JWT từ cookie — không bắt đầu trước Nhóm A, lý do đã ghi ở mục 3. **Chưa bắt đầu**: `requirements.txt` chưa có thư viện JWT nào, `organization_id` vẫn là chuỗi `"default"` chứ không phải UUID tổ chức thật. Kế hoạch chi tiết ở `RA_SOAT_DONG_BO_BA_REPO.md` Đợt 3 | B1, Nhóm A |
