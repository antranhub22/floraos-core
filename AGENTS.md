...
- **Tiêu chuẩn Dẫn chứng Video Kép & Chống Tràn Từ Khóa Thô của Market Intelligence (Evidence & Clean Phrasing Standard)**:
  - **Dẫn chứng Video Kép (Dual Video Evidence)**: Mọi thẻ cơ hội / xu hướng / kịch bản trên giao diện Market Intelligence bắt buộc hiển thị cặp thumbnail dẫn chứng video thực tế từ 2 nền tảng: **TikTok** (video dọc 9:16, badge TikTok neon, tương tác thật, tên kênh hoa thật) và **YouTube** (video ngang 16:9, badge YT đỏ, tên kênh thật), có nút Play overlay và mở tab mới khi nhấp chuột (`window.open(url, "_blank")`). Quản lý tập trung tại `src/components/market-intelligence/video-evidence-catalog.ts`.
...
Trạng thái hiện tại: **Nâng cấp Xem Video Trực Tiếp Tại Chỗ Bảng Vàng Xu Hướng & Cập Nhật Dữ Liệu Thời Gian Thực Market Intelligence (24/09 trưa)** — Bảng Vàng Quán Quân Chu Kỳ ([`top-champions-block.tsx`](file:///Users/tuan/Projects/floraos-core/src/components/market-intelligence/top-champions-block.tsx)) tích hợp phát video trực tiếp tại chỗ qua modal chuyên dụng ([`video-preview-modal.tsx`](file:///Users/tuan/Projects/floraos-core/src/components/market-intelligence/video-preview-modal.tsx), YouTube embed HD `autoplay=1`, không reload/nhảy tab, đóng bằng Escape/backdrop/X); nhấp "Chi tiết →" giữ nguyên mở Drawer kịch bản; cập nhật SerpApi Key thời gian thực mới, kích hoạt lượt chạy `DAILY_DEEP` nạp 70 cơ hội mới; thay thế toàn bộ video cũ (4 năm, 7 năm) bằng mẫu video tuần này ([`video-catalog-presets.ts`](file:///Users/tuan/Projects/floraos-core/src/modules/market-intelligence/domain/video-catalog-presets.ts)); backfill 4.318 bản ghi CSDL; lọc bỏ video spam khỏi YouTube adapter. `npm test` **806/806 xanh (100/100 tệp)** · `tests/unit/market-intelligence/` **50/50 xanh (10/10 tệp)** · `tsc --noEmit` **sạch 100%**.

Song song (Creative Studio): **Khu vực C (Audio) hoàn thiện 4 tác vụ (24/09 tối)** — Voiceover/Music Select/Audio Mix/Voice Clone khác nhau thật, credit theo bảng giá, thư viện nhạc có giấy phép (`music_tracks`), Voice Clone ElevenLabs (`voice_clones`, job `audio.voice_clone`); `npm test` **824/824**; cần `ELEVENLABS_API_KEY` + migrate `20260924120000`; nợ mới #128 — chi tiết `docs/kien-truc/TRANG_THAI.md` mục 1.

Trước đó: **Creative Studio chạy thật & nâng cấp (24/09, nhánh `fix/creative-studio-production-ready`)** — kịch bản bối cảnh theo chủ đề sinh ở Chặng 05 (`creative.scene_plan`, AIC-18) dùng chung B/C/D/E; sửa chất lượng ảnh D (thân sản phẩm đặc, khử viền dải 6px, cổng phủ ≥ 60%); E dựng storyboard theo kịch bản, có giọng đọc, phát video ký có hạn; B tự lưu bài (`content_drafts`); Chặng 07 xem lại và sửa tại chỗ ảnh/bài/âm thanh/video (`scene-revisions`, `content-rewrites`). `npm test` **792/792** · `tsc` sạch · `check:docs` khớp. Máy dev cần `prisma migrate deploy` + `db:seed`. Trước đó: **Rà soát & sửa Creative Studio 14 chặng (23/09 tối)** — chi tiết ở `docs/kien-truc/TRANG_THAI.md` mục 1. Nhánh Cloud M04b vào hàng đợi (`media.variant.cloud`), sửa phép đo Subject Integrity (co biên 5px), nối worker `audio.generate`, Khu vực F + Chặng 10–14 chạy trên dữ liệu thật (bảng `campaign_packages`). `npm test` **770/770** · `tsc` sạch · `check:docs` khớp. **`npm run test:tenant` chưa chạy lại** — bắt buộc trước merge.

Trạng thái trước đó (ĐÃ ĐÍNH CHÍNH 23/09 tối — xem TRANG_THAI mục 1; ba điều "triệt tiêu lỗi trùng 1 ảnh", "fallback client canvas cutout", "~0,46s" không mô tả đúng mã lúc ấy): **Hoàn thiện Khung Phân Cảnh Narrative Arc 4 Chặng & Động cơ Local Studio Backdrop Engine M04b Creative Studio (23/09) hoàn tất 100%, nghiệm thu xong trên máy thật:** `npm test` **747/747 xanh (92/92 tệp)** · `npm run test:tenant` **206/206 xanh thật (27/27 tệp)** · `npx tsc --noEmit` **SẠCH 100%**. Hệ thống Creative Studio (Khu vực D - M04b Biến thể Marketing) được hoàn thiện toàn diện:
- **Chuẩn hóa 4 Khung Phân Cảnh Narrative Arc**: Cảnh 1 (SETUP: Studio Trắng Tinh Khôi `clean_white` đổ bóng 2 tầng), Cảnh 2 (RISING: Không gian Lifestyle Sang Trọng `boutique_bokeh` / sảnh tiệc mừng khách sạn), Cảnh 3 (CLIMAX: Gỗ Tối Giản Nghệ Thuật Bắc Âu `wood_warm` tôn vinh thiệp chúc mừng OCR và nơ thiết kế riêng), Cảnh 4 (CTA: Tách Nền Trong Suốt PNG `transparent` bằng U2-Net / Rembg alpha mask).
- **Triệt tiêu lỗi "Trùng lặp 1 bức ảnh duy nhất"**: Trước đây khi các Cloud Provider (Stability AI 402, Fal 403, Gemini 429) hết credit/quota, chuỗi router rơi về `studio_local`, nhưng adapter chỉ trả lại bytes ảnh gốc chưa xử lý. Nay nâng cấp `StudioLocalImageProvider` kết nối trực tiếp với Python `StudioBackdropEngine` và `RembgSegmenter` qua `generate_scene.py`.
- **Bộ nhớ đệm RGBA (`.rgba.png`) & Tốc độ ~0.46s**: Lưu cache mặt nạ bóc tách lần đầu; các lượt ghép phân cảnh tiếp theo chỉ mất **~0.46s / ảnh $2048 \times 2048$**, 0 VNĐ / 0 Token API, chạy 100% offline.
- **Client-Side Tự động đồng bộ**: `variant-workspace.tsx` tự động truy vấn CSDL `assets` (`kind=MARKETING`) nạp ngay các phân cảnh có sẵn khi tải trang; hỗ trợ nút bấm sinh độc lập từng Cảnh 2, Cảnh 3, Cảnh 4 với trạng thái loading riêng (`generatingSceneIndex`). Cảnh 4 tích hợp cơ chế kép API Rembg + fallback client canvas cutout.

Trạng thái trước đó: **Nâng cấp Bóc tách Phụ liệu & Thiệp Chúc Mừng OCR Đồng Bộ Toàn Hệ Thống (20/09) hoàn tất 100%, nghiệm thu xong trên máy thật:** `npm test` **575/575 xanh** · `npm run test:tenant` **206/206 xanh thật (27/27 tệp)** · `npx vitest run tests/unit/market-intelligence/` **33/33 xanh (8/8 tệp)** · `npx tsc --noEmit` **SẠCH 100%**. Hệ thống bóc tách thị giác sản phẩm hoa tươi bằng OpenAI Multimodal Vision (`gpt-4o-mini`) được nâng cấp toàn diện để phân rã nguyên tử (Atomic Disaggregated Fields) phụ liệu: lá & cành đệm (tên lá, số cành, màu sắc, vai trò đệm), thiệp & biển chúc mừng (bóc tách nguyên văn chữ in/viết `printedText` qua OCR thị giác để phân loại chính xác mục đích sử dụng), nơ & ruy băng (chất liệu, màu sắc, kiểu dáng), giấy gói bao bì (chất liệu, màu sắc), phụ kiện decor đặc biệt. Đồng bộ 100% trên cả Chặng 02 Product Intelligence (`/thi-truong`), Tab M01a & Thẻ chào Zalo M01c (`/tai-anh`), và các tầng Domain Model (`ProductPackaging`, `synthesizeProductResearchQueries`, `CommercialPassport`, `mapAnalysisFromSchema`).

Trạng thái trước đó: **Market Intelligence & Product Intelligence Vision AI hoàn tất 100% (20/09), nghiệm thu xong trên máy thật:** `npm test` **540/540 xanh** · `npm run test:tenant` **206/206 xanh thật (27/27 tệp)** · `npx vitest run tests/unit/market-intelligence/` **32/32 xanh (8/8 tệp)** · `npx tsc --noEmit` **SẠCH 100%**. Hệ thống bóc tách thị giác sản phẩm hoa tươi bằng OpenAI Multimodal Vision (`gpt-4o-mini`, đọc Base64 Data URL trực tiếp, nhận diện đúng 100% ảnh bó hoa hồng đỏ, tulip, mẫu đơn... gỡ bỏ triệt để fallback cứng "Hoa hồng kem dâu"). Cơ chế tương tác lựa chọn tiến trình từng bước (Progressive Selection): 01. BRING (Tải ảnh/Catalog) → 02. UNDERSTAND (Vision AI bóc tách nguyên tử, tick chọn hướng từ khóa do Synthesizer sinh ra) → 03. DISCOVER (Trend Fit Matrix, điểm số, 3 vùng KEEP/IMPROVE/TEST) → 04. IDEATE (10 chủ đề kèm Dẫn chứng Video Kép TikTok 9:16 + YouTube 16:9, click chọn chủ đề trọng tâm) → 05. CHOOSE/HANDOFF (bàn giao 1-chạm sang AI Video Studio và Studio Biến Thể Ảnh). SSOT quy trình: `docs/dac-ta/FLORAOS_PRODUCT_TO_MARKET_USER_JOURNEY.md` và `docs/kien-truc/FLORAOS_MARKET_INTELLIGENCE_ARCHITECTURE.md`.

Trạng thái trước đó: **P24 — M04b về đúng kiến trúc job (09/17), nghiệm thu xong trên máy thật:** `npm test` 479/479 · `npm run test:tenant` **200/200, 25/25 tệp** · `pytest` worker 229/229 · `npx tsc --noEmit` sạch. *(Con số của mốc P24. Đo lại 18/09 trên cây làm việc hiện tại: `npm test` **525/525, 69 tệp**, `tsc` sạch. Đừng chép con số ở đây đi đâu — chạy lại mà lấy.)* Đóng `POST /api/v1/media/background-removal` (không auth, không RBAC, không `organization_id`, không credit, `spawn` Python + parse stdout, SSRF qua `image_url` tuỳ ý). Dựng `media.variant` qua `enqueueJob`: bốn route `/api/v1/media/variants*`, cặp năng lực `I4` (chạy) ↔ `I5` (duyệt, trần cứng) — **143 mã / 39 trần cứng**, worker `workers/media_ai/jobs/variant_worker.py` với bốn `stage` thật, biến thể ghi thành `assets` `kind = MARKETING` / `PENDING` có cha là Master đã duyệt, watermark lấy logo thật từ `brand_profiles.logo_asset_id`. Cổng **Subject Integrity** đo tỷ lệ điểm ảnh lõi chủ thể còn trùng khít với Master Image (mặt nạ co biên), ngưỡng 0,999 / 0,99, `REJECTED` thì không ghi asset nào — thay ba hằng số `100/99/98` gõ tay trong giao diện. Gỡ bộ dựng canvas phía trình duyệt và ảnh mẫu Unsplash trong chuỗi lùi. Nợ mới #76–#80. Chi tiết ở `docs/dac-ta/Checklist_Thuc_Thi.md` mục P24.
